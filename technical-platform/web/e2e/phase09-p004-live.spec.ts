...
test('P004 real generic request enforces scope, dual review, independent acceptance and three-portal projection', async ({ page, request }) => {
  ...
  await expect(page.getByRole('heading', { name: '通用申请与审批', level: 1 })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '通用申请审批与执行', level: 1 })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '通用流程实例监控', level: 1 })).toBeVisible()
  ...
  await expect(techRecord.locator('.actions button')).toHaveCount(0)
