...
/**
 * P016 notifications expose workflow metadata only; private facts, amounts and receipts stay
 * server-side.
 */
public final class Phase11P016NotificationHandler implements PlatformOutboxHandler {

  public static final String EVENT_TYPE = "P016_CARE_EVENT", AGGREGATE_TYPE = "P016_CARE";

  private static final String TEMPLATE = "P016_CARE_EVENT";
  ...
  public Phase11P016NotificationHandler(
      NotificationService notifications, JdbcTemplate jdbc, ObjectMapper mapper) {
    if (notifications == null || jdbc == null || mapper == null) {
      throw new IllegalArgumentException("P016 notification dependencies are required");
    }
  ...
  public String consumerName() {
    return "phase11-p016-notification";
  }
  ...
  public void handle(PlatformOutboxEvent event) {
    if (event == null
      ...
      throw new IllegalArgumentException("P016 outbox aggregate is invalid");
    }
    ...
    for (UUID recipient : recipients(p.path("recipientEmployeeIds"))) {
      notifications.create(
          new NotificationService.CreateCommand(
              event.tenantId(),
              null,
              "p016-notify:" + event.id() + ":" + recipient,
              TEMPLATE,
              "IN_APP",
              "EMPLOYEE",
              recipient,
...
""",
  ...
  private JsonNode parse(String raw) {
    try {
      ...
      if (n == null || !n.isObject()) {
        throw new IllegalArgumentException("P016 event payload must be an object");
      }
      return n;
    } catch (IllegalArgumentException e) {
      throw e;
    } catch (Exception e) {
      throw new IllegalArgumentException("P016 event payload is invalid JSON", e);
    }
  }
  ...
  private static String required(JsonNode n, String f) {
    ...
    if (v == null || !v.isTextual() || v.asText().isBlank()) {
      throw new IllegalArgumentException("P016 event field is required: " + f);
    }
    return v.asText();
  }
      ...
      n.forEach(
          v -> {
            try {
              ...
              throw new IllegalArgumentException("P016 recipient is invalid", e);
            }
          });
    }
    return ids;
  ...
  private static String label(String n) {
    return switch (n) {
      case "S01" -> "Application";
      case "S02" -> "Eligibility";
      case "S03" -> "Privacy consent";
      case "S04" -> "Approval";
      case "S05" -> "External execution";
      case "S06" -> "Employee confirmation";
      case "S07" -> "Reconciliation";
      case "S08" -> "Archive";
      case "END" -> "Closed";
      default -> throw new IllegalArgumentException("P016 event node is not source-backed: " + n);
    };
  }
}
