...
public final class P007ShiftController {
    public static final String CREATE="p007.schedule.create",READ="p007.schedule.read",MANAGE="p007.schedule.manage",SUBMIT="p007.shift.submit",CONFIRM="p007.shift.confirm",REVIEW="p007.shift.review",MONITOR="p007.schedule.monitor";
    ...
    public ShiftChangeService.Aggregate create(@AuthenticationPrincipal SessionPrincipal p,@RequestHeader("Idempotency-Key")String key,@RequestBody ShiftChangeService.CreateCommand c){require(auth.authorizeAction(p.context(),CREATE));require(auth.authorizeData(p.context(),CREATE,new AuthorizationTarget(p.context().tenantId(),c.targetEmployeeId(),c.ownerCenterId(),p.context().positionId(),c.targetEmployeeId())));audit.recordOperation(p.context(),"P007_CREATE_ATTEMPT","attendance.shift_change_request",null);var r=shifts.create(ctx(p),key,hash(c),c);audit.recordOperation(p.context(),"P007_CREATED","attendance.shift_change_request",r.record().id());return r;}
    ...
    public ShiftChangeService.Aggregate action(@AuthenticationPrincipal SessionPrincipal p,@PathVariable UUID id,@PathVariable String actionCode,@RequestHeader("Idempotency-Key")String key,@RequestBody ShiftChangeService.ActionCommand c){String action=safe(actionCode);String perm=permission(action);require(auth.authorizeAction(p.context(),perm));var current=shifts.find(ctx(p),id).orElseThrow(()->new IllegalArgumentException("P007 record not found"));require(auth.authorizeData(p.context(),perm,target(current.record(),p)));if((CONFIRM.equals(perm)||SUBMIT.equals(perm))&&!p.context().employeeId().equals(current.record().targetEmployeeId()))throw denied("P007 employee action is self-only");audit.recordOperation(p.context(),"P007_ACTION_ATTEMPT_"+action,"attendance.shift_change_request",id);var r=shifts.act(ctx(p),id,action,key,hash(Map.of("action",action,"body",c)),c);audit.recordOperation(p.context(),"P007_ACTION_"+action,"attendance.shift_change_request",id);return r;}

    private List<ShiftChangeService.Aggregate> listVisible(SessionPrincipal p){boolean read=allowed(p,READ),manage=allowed(p,MANAGE),monitor=allowed(p,MONITOR);if(!read&&!manage&&!monitor&&!allowed(p,CONFIRM)&&!allowed(p,SUBMIT))throw denied("no P007 read surface");List<ShiftChangeService.Aggregate> result=shifts.list(ctx(p)).stream().map(r->project(p,r)).filter(java.util.Objects::nonNull).toList();audit.recordOperation(p.context(),"P007_LIST","attendance.shift_change_request",null);return result;}
    private ShiftChangeService.Aggregate project(SessionPrincipal p,ShiftChangeService.Aggregate a){var r=a.record();if(allowed(p,MANAGE)&&auth.authorizeData(p.context(),MANAGE,target(r,p)).allowed())return a;if((allowed(p,READ)||allowed(p,CONFIRM)||allowed(p,SUBMIT))&&p.context().employeeId().equals(r.targetEmployeeId())&&auth.authorizeData(p.context(),READ,target(r,p)).allowed())return a;if(allowed(p,MONITOR)&&auth.authorizeData(p.context(),MONITOR,target(r,p)).allowed())return a.metadataOnly();return null;}
    private static String permission(String a){return switch(a){case"CONFIRM_SCHEDULE"->CONFIRM;case"SUBMIT_SHIFT_CHANGE"->SUBMIT;case"APPROVE_CHANGE","RETURN_CHANGE"->REVIEW;default->MANAGE;};}
