...
public final class P009OvertimeController {
  ...
  private static final String SUBMIT = "p009.overtime.submit",
      READ = "p009.overtime.read",
      MONITOR = "p009.overtime.monitor";
  ...
  public OvertimeService.Overtime create(
      @AuthenticationPrincipal SessionPrincipal p,
      @RequestHeader("Idempotency-Key") String key,
      @RequestBody OvertimeService.CreateCommand c) {
    require(authorization.authorizeAction(p.context(), SUBMIT));
    require(authorization.authorizeData(p.context(), SUBMIT, self(p)));
    audit.recordOperation(p.context(), "P009_CREATE_ATTEMPT", "attendance.overtime_request", null);
    var r = overtime.create(context(p), key, hash(c), c);
    audit.recordOperation(p.context(), "P009_CREATED", "attendance.overtime_request", r.id());
    return r;
  }

  @GetMapping("/overtime-requests/{id}")
  public OvertimeService.Overtime get(
      @AuthenticationPrincipal SessionPrincipal p, @PathVariable UUID id) {
    String permission = readPermission(p);
    var v =
        overtime
            .find(context(p), id)
            .orElseThrow(() -> new IllegalArgumentException("P009 overtime request not found"));
    require(authorization.authorizeData(p.context(), permission, target(v)));
    audit.recordOperation(p.context(), "P009_READ", "attendance.overtime_request", id);
    return view(p, v);
  }

  @GetMapping("/overtime-requests")
  public List<OvertimeService.Overtime> list(@AuthenticationPrincipal SessionPrincipal p) {
    String permission = readPermission(p);
    var r =
        overtime.list(context(p)).stream()
            .filter(v -> authorization.authorizeData(p.context(), permission, target(v)).allowed())
            .map(v -> view(p, v))
            .toList();
    audit.recordOperation(p.context(), "P009_LIST", "attendance.overtime_request", null);
    return r;
  }

  @PostMapping("/overtime-requests/{id}/actions/{actionCode}")
  public OvertimeService.Overtime act(
      @AuthenticationPrincipal SessionPrincipal p,
      @PathVariable UUID id,
      @PathVariable String actionCode,
      @RequestHeader("Idempotency-Key") String key,
      @RequestBody OvertimeService.ActionCommand c) {
    var current =
        overtime
            .find(context(p), id)
            .orElseThrow(() -> new IllegalArgumentException("P009 overtime request not found"));
    String permission = overtime.permissionForNode(current), action = safe(actionCode);
    ...
    audit.recordOperation(
        p.context(), "P009_ACTION_ATTEMPT_" + action, "attendance.overtime_request", id);
    var r =
        overtime.act(context(p), id, action, key, hash(Map.of("actionCode", action, "body", c)), c);
    audit.recordOperation(p.context(), "P009_ACTION_" + action, "attendance.overtime_request", id);
    return view(p, r);
  }

  private OvertimeService.Overtime view(SessionPrincipal p, OvertimeService.Overtime v) {
  ...
  private static AuthorizationTarget self(SessionPrincipal p) {
  ...
  private static AuthorizationTarget target(OvertimeService.Overtime v) {
    return new AuthorizationTarget(
  ...
  private static void require(AuthorizationDecision d) {
    if (!d.allowed()) {
      throw new AccessDeniedException("P009 authorization denied: " + d.reason());
    }
  }
  ...
  private String hash(Object v) {
    try {
      ...
      throw new IllegalArgumentException("P009 request cannot be hashed", e);
    }
  }
}
