...
/**
 * P015 notifications expose workflow metadata, never point values, ranks, evidence or rule
 * thresholds.
 */
public final class Phase11P015NotificationHandler implements PlatformOutboxHandler {

  public static final String EVENT_TYPE = "P015_POINT_EVENT", AGGREGATE_TYPE = "P015_POINTS";

  private static final String TEMPLATE = "P015_POINT_EVENT";
  ...
  public Phase11P015NotificationHandler(
      NotificationService notifications, JdbcTemplate jdbc, ObjectMapper mapper) {
    if (notifications == null || jdbc == null || mapper == null) {
      throw new IllegalArgumentException("P015 notification dependencies are required");
    }
  ...
  public String consumerName() {
    return "phase11-p015-notification";
  }
  ...
  public void handle(PlatformOutboxEvent event) {
    if (event == null
      ...
      throw new IllegalArgumentException("P015 outbox aggregate is invalid");
    }
    ...
    for (UUID recipient : recipients(p.path("recipientEmployeeIds"))) {
      notifications.create(
          new NotificationService.CreateCommand(
              event.tenantId(),
              null,
              "p015-notify:" + event.id() + ":" + recipient,
              TEMPLATE,
              "IN_APP",
              "EMPLOYEE",
              recipient,
...
""",
  ...
  private JsonNode parse(String raw) {
    try {
      ...
      if (n == null || !n.isObject()) {
        throw new IllegalArgumentException("P015 event payload must be an object");
      }
      return n;
    } catch (IllegalArgumentException e) {
      throw e;
    } catch (Exception e) {
      throw new IllegalArgumentException("P015 event payload is invalid JSON", e);
    }
  }
  ...
  private static String required(JsonNode n, String f) {
    ...
    if (v == null || !v.isTextual() || v.asText().isBlank()) {
      throw new IllegalArgumentException("P015 event field is required: " + f);
    }
    return v.asText();
  }
      ...
      n.forEach(
          v -> {
            try {
              ...
              throw new IllegalArgumentException("P015 recipient is invalid", e);
            }
          });
    }
    return ids;
