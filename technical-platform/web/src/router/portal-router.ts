...
function phase09P002Routes(portal: PortalDefinition): RouteRecordRaw[] {
  if (portal.code === 'employee') return [
    { path: '/employee/03/07/04', name: 'p002-temporary-permission-request', component: P002PermissionRequestPage,
      props: { portal, mode: 'employee', requestKind: 'TEMPORARY_PERMISSION' }, meta: { permissionsAny: ['p002.request.submit', 'p002.request.read'] } },
    { path: '/employee/03/07/05', name: 'p002-project-permission-request', component: P002PermissionRequestPage,
      props: { portal, mode: 'employee', requestKind: 'PROJECT_PERMISSION' }, meta: { permissionsAny: ['p002.request.submit', 'p002.request.read'] } },
  ]
  if (portal.code === 'tech') return [{ path: '/tech/03/01/04', name: 'p002-permission-execution', component: P002PermissionRequestPage,
    props: { portal, mode: 'tech' }, meta: { permissionsAny: ['p002.request.read', 'p002.request.execute', 'p002.request.revoke'] } }]
  return []
}
...
function phase09P003Routes(portal: PortalDefinition): RouteRecordRaw[] {
  if (portal.code === 'employee') return [{
    ...
    props: { portal, mode: 'employee' }, meta: { permissionsAny: ['p003.change.submit', 'p003.change.read'] },
  }]
  if (portal.code === 'center') return [{
    path: '/center/03/02/01', name: 'p003-profile-roster', component: P003ProfileChangePage,
    props: { portal, mode: 'center' }, meta: { permissionsAny: ['p003.change.read', 'p003.change.review'] },
  }]
  if (portal.code === 'tech') return [{
    path: '/tech/04/01/01', name: 'p003-profile-sync-monitor', component: P003ProfileChangePage,
    props: { portal, mode: 'tech' }, meta: { permissionsAny: ['p003.change.read', 'p003.change.apply'] },
  }]
  return []
}
...
function phase09P004Routes(portal: PortalDefinition): RouteRecordRaw[] {
  if (portal.code === 'employee') return [{
    ...
    props: { portal, mode: 'employee' }, meta: { permissionsAny: ['p004.request.submit', 'p004.request.read'] },
  }]
  return []
}
...
function phase09P005Routes(portal: PortalDefinition): RouteRecordRaw[] {
  if (portal.code === 'employee') return [{
    ...
    props: { portal, mode: 'employee' }, meta: { permissionsAny: ['p005.notice.read', 'p005.notice.receipt'] },
  }]
  if (portal.code === 'center') return [{
    path: '/center/13/01/05', name: 'p005-notice-publish', component: P005NoticePage,
    props: { portal, mode: 'center' }, meta: { permissionsAny: ['p005.notice.publish', 'p005.notice.manage'] },
  }]
  return []
}
...
function phase09SharedCenterRoutes(portal: PortalDefinition): RouteRecordRaw[] {
  ...
  return [{
    ...
    meta: { permissionsAny: [
      'p001.session.monitor', 'p002.request.read', 'p002.request.review', 'p003.change.read', 'p003.change.review',
      'p004.request.read', 'p004.request.act',
    ] },
  }]
}
...
function phase09SharedTechWorkflowRoutes(portal: PortalDefinition): RouteRecordRaw[] {
  if (portal.code !== 'tech') return []
  return [{
    path: '/tech/05/03/01', name: 'p004-workflow-instance-monitor', component: Phase09TechWorkflowMonitorPage, props: { portal },
    meta: { permissionsAny: ['p004.request.read', 'p005.notice.monitor'] },
  }]
}

function authenticatedRoutes(portal: PortalDefinition): RouteRecordRaw {
  return {
    path: '/', component: AuthenticatedPortalLayout, props: { portal }, meta: { requiresAuth: true },
    children: [
      ...
      ...phase09P005Routes(portal), ...phase09SharedCenterRoutes(portal), ...phase09SharedTechWorkflowRoutes(portal),
