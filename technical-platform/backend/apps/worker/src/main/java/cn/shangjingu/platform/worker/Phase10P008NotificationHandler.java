...
/**
 * P008 leave progress projection. Reasons, quota account identifiers and immutable evidence never
 * enter messages.
 */
public final class Phase10P008NotificationHandler implements PlatformOutboxHandler {

  public static final String EVENT_TYPE = "P008_LEAVE_EVENT", AGGREGATE_TYPE = "P008_LEAVE";

  private static final String TEMPLATE = "P008_LEAVE_EVENT";
  ...
  public Phase10P008NotificationHandler(
      NotificationService notifications, JdbcTemplate jdbc, ObjectMapper mapper) {
    if (notifications == null || jdbc == null || mapper == null) {
      throw new IllegalArgumentException("P008 notification dependencies are required");
    }
  ...
  public String consumerName() {
    return "phase10-p008-notification";
  }
  ...
  public void handle(PlatformOutboxEvent event) {
    if (event == null
      ...
      throw new IllegalArgumentException("P008 outbox aggregate is invalid");
    }
    ...
    for (UUID recipient : recipients(p.path("recipientEmployeeIds"))) {
      notifications.create(
          new NotificationService.CreateCommand(
              event.tenantId(),
              null,
              "p008-notify:" + event.id() + ":" + recipient,
              TEMPLATE,
              "IN_APP",
              "EMPLOYEE",
              recipient,
...
""",
  ...
  private JsonNode parse(String raw) {
    try {
      ...
      if (n == null || !n.isObject()) {
        throw new IllegalArgumentException("P008 event payload must be an object");
      }
      return n;
    } catch (IllegalArgumentException e) {
      throw e;
    } catch (Exception e) {
      throw new IllegalArgumentException("P008 event payload is invalid JSON", e);
    }
  }
  ...
  private static String required(JsonNode n, String f) {
    ...
    if (v == null || !v.isTextual() || v.asText().isBlank()) {
      throw new IllegalArgumentException("P008 event field is required: " + f);
    }
    return v.asText();
  }
      ...
      n.forEach(
          v -> {
            try {
              ...
              throw new IllegalArgumentException("P008 recipient is invalid", e);
            }
          });
    }
    return ids;
  ...
  private static String label(String n) {
    return switch (n) {
      ...
      case "S10" -> "考勤日结与归档";
      case "END" -> "已关闭";
      default -> throw new IllegalArgumentException("P008 event node is not source-backed: " + n);
    };
  }
}
