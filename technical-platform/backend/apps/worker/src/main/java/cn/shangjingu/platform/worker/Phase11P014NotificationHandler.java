...
public final class Phase11P014NotificationHandler implements PlatformOutboxHandler {
  ...
  public static final String EVENT_TYPE = "P014_DISCIPLINE_EVENT",
      AGGREGATE_TYPE = "P014_DISCIPLINE";

  private static final String TEMPLATE = "P014_DISCIPLINE_EVENT";
  ...
  public Phase11P014NotificationHandler(
      NotificationService notifications, JdbcTemplate jdbc, ObjectMapper mapper) {
    if (notifications == null || jdbc == null || mapper == null) {
      throw new IllegalArgumentException("P014 notification dependencies are required");
    }
  ...
  public String consumerName() {
    return "phase11-p014-notification";
  }
  ...
  public void handle(PlatformOutboxEvent event) {
    if (event == null
      ...
      throw new IllegalArgumentException("P014 outbox aggregate is invalid");
    }
    ...
    for (UUID recipient : recipients(p.path("recipientEmployeeIds"))) {
      notifications.create(
          new NotificationService.CreateCommand(
              event.tenantId(),
              null,
              "p014-notify:" + event.id() + ":" + recipient,
              TEMPLATE,
              "IN_APP",
              "EMPLOYEE",
              recipient,
...
""",
  ...
  private JsonNode parse(String raw) {
    try {
      ...
      if (n == null || !n.isObject()) {
        throw new IllegalArgumentException("P014 event payload must be an object");
      }
      return n;
    } catch (IllegalArgumentException e) {
      throw e;
    } catch (Exception e) {
      throw new IllegalArgumentException("P014 event payload is invalid JSON", e);
    }
  }
  ...
  private static String required(JsonNode n, String f) {
    ...
    if (v == null || !v.isTextual() || v.asText().isBlank()) {
      throw new IllegalArgumentException("P014 event field is required: " + f);
    }
    return v.asText();
  }
      ...
      n.forEach(
          v -> {
            try {
              ...
              throw new IllegalArgumentException("P014 recipient is invalid", e);
            }
          });
    }
    return ids;
  ...
  private static String label(String n) {
    return switch (n) {
      ...
      case "END" -> "Closed";
      default -> throw new IllegalArgumentException("P014 event node is not source-backed: " + n);
    };
  }
}
