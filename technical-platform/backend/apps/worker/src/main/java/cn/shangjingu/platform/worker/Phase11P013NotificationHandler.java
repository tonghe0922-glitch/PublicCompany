...
/**
 * P013 notifications contain workflow metadata only; amounts, evidence and receipt references
 * remain server-side.
 */
public final class Phase11P013NotificationHandler implements PlatformOutboxHandler {

  public static final String EVENT_TYPE = "P013_REWARD_EVENT", AGGREGATE_TYPE = "P013_REWARD";

  private static final String TEMPLATE = "P013_REWARD_EVENT";
  ...
  public Phase11P013NotificationHandler(
      NotificationService notifications, JdbcTemplate jdbc, ObjectMapper mapper) {
    if (notifications == null || jdbc == null || mapper == null) {
      throw new IllegalArgumentException("P013 notification dependencies are required");
    }
  ...
  public String consumerName() {
    return "phase11-p013-notification";
  }
  ...
  public void handle(PlatformOutboxEvent event) {
    if (event == null
      ...
      throw new IllegalArgumentException("P013 outbox aggregate is invalid");
    }
    ...
    for (UUID recipient : recipients(p.path("recipientEmployeeIds"))) {
      notifications.create(
          new NotificationService.CreateCommand(
              event.tenantId(),
              null,
              "p013-notify:" + event.id() + ":" + recipient,
              TEMPLATE,
              "IN_APP",
              "EMPLOYEE",
              recipient,
...
""",
  ...
  private JsonNode parse(String raw) {
    try {
      ...
      if (n == null || !n.isObject()) {
        throw new IllegalArgumentException("P013 event payload must be an object");
      }
      return n;
    } catch (IllegalArgumentException e) {
      throw e;
    } catch (Exception e) {
      throw new IllegalArgumentException("P013 event payload is invalid JSON", e);
    }
  }
  ...
  private static String required(JsonNode n, String f) {
    ...
    if (v == null || !v.isTextual() || v.asText().isBlank()) {
      throw new IllegalArgumentException("P013 event field is required: " + f);
    }
    return v.asText();
  }
      ...
      n.forEach(
          v -> {
            try {
              ...
              throw new IllegalArgumentException("P013 recipient is invalid", e);
            }
          });
    }
    return ids;
  ...
  private static String label(String n) {
    return switch (n) {
      ...
      case "S09" -> "Archive";
      case "END" -> "Closed";
      default -> throw new IllegalArgumentException("P013 event node is not source-backed: " + n);
    };
  }
}
