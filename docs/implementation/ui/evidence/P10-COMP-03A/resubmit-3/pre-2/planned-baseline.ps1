...
$plannedPaths = @(
    ...
    'technical-platform/web/src/showcase/ShowcaseApp.vue',
...
$manifest = [ordered]@{
    schema_version = '1.0'
    kind = 'P10-COMP-03A-resubmit-3-pre-2-baseline'
    repository_root = $root
    generated_at_utc = (Get-Date).ToUniversalTime().ToString('o')
    git_present = Test-Path -LiteralPath (Join-Path $root '.git')
    ...
    runtime_boundary = 'pre-existing active runtime / source-baseline non-writer'
