...
public final class Phase11P011BrowserBackendFixture {
    ...
    try (Connection connection =
      ...
      for (int index = 0; index < roles.length; index++) {
        seedActor(
            ...
            roles[index][2],
            hash,
            index == 6 ? CENTER_B : CENTER_A,
            index == 6 ? POSITION_B : POSITION_A);
      }
    }
  }
  ...
  private static void writeFacts(PostgreSQLContainer<?> postgres, GenericContainer<?> redis)
    ...
    Path output =
        root()
            .resolve(
                "technical-platform/backend/apps/api/target/phase11-p011-fixture-runtime.json");
  ...
  private static UUID derived(int group, int index) {
    return uuid("%d0000000-0000-0000-0000-%012d".formatted(group, 3321 + index));
  }
