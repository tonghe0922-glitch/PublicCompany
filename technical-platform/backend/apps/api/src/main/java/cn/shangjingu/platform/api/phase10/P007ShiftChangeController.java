...
public final class P007ShiftChangeController {
  ...
  private static final String READ = "p007.schedule.read",
      MANAGE = "p007.schedule.manage",
      CHANGE = "p007.schedule.change",
      REVIEW = "p007.schedule.review",
      MONITOR = "p007.schedule.monitor";

  private final ShiftChangeService shifts;
  ...
  public P007ShiftChangeController(
      ShiftChangeService shifts,
  ...
  public ShiftChangeService.ShiftChange create(
      @AuthenticationPrincipal SessionPrincipal principal,
      @RequestHeader("Idempotency-Key") String key,
      @RequestBody ShiftChangeService.CreateCommand command) {
    ...
    audit.recordOperation(
        principal.context(), "P007_CREATE_ATTEMPT", "attendance.shift_change_request", null);
    var result = shifts.create(context(principal), key, hash(command), command);
    audit.recordOperation(
        principal.context(), "P007_CREATED", "attendance.shift_change_request", result.id());
    return view(principal, result);
  }

  @GetMapping("/shift-changes/{id}")
  public ShiftChangeService.ShiftChange get(
    ...
    var value =
        shifts
            ...
            .orElseThrow(() -> new IllegalArgumentException("P007 shift change not found"));
    require(authorization.authorizeData(principal.context(), permission, target(value)));
    audit.recordOperation(principal.context(), "P007_READ", "attendance.shift_change_request", id);
    return view(principal, value);
  }

  @GetMapping("/shift-changes")
  public List<ShiftChangeService.ShiftChange> list(
      @AuthenticationPrincipal SessionPrincipal principal) {
    return scoped(principal, false);
  }

  @GetMapping("/schedules")
  public List<ShiftChangeService.ShiftChange> schedules(
      @AuthenticationPrincipal SessionPrincipal principal) {
    return scoped(principal, true);
  }

  @PostMapping("/shift-changes/{id}/actions/{actionCode}")
  public ShiftChangeService.ShiftChange act(
      @AuthenticationPrincipal SessionPrincipal principal,
      @PathVariable UUID id,
      @PathVariable String actionCode,
      @RequestHeader("Idempotency-Key") String key,
      @RequestBody ShiftChangeService.ActionCommand command) {
    var current =
        shifts
            .find(context(principal), id)
            .orElseThrow(() -> new IllegalArgumentException("P007 shift change not found"));
    String action = safe(actionCode),
        permission =
            shifts.employeeNode(current) ? CHANGE : (shifts.reviewNode(current) ? REVIEW : MANAGE);
    ...
    var result =
        shifts.act(
            ...
            command);
    audit.recordOperation(
        principal.context(), "P007_ACTION_" + action, "attendance.shift_change_request", id);
    return view(principal, result);
  }

  private List<ShiftChangeService.ShiftChange> scoped(SessionPrincipal p, boolean schedules) {
    String permission = readPermission(p);
    List<ShiftChangeService.ShiftChange> source =
        schedules ? shifts.schedules(context(p)) : shifts.list(context(p));
    var result =
        source.stream()
    ...
    audit.recordOperation(
        p.context(),
        schedules ? "P007_SCHEDULE_LIST" : "P007_LIST",
        "attendance.shift_change_request",
        null);
    return result;
  }

  private ShiftChangeService.ShiftChange view(
      SessionPrincipal p, ShiftChangeService.ShiftChange v) {
    if (p.context().employeeId().equals(v.ownerEmployeeId())
        || authorization.authorizeAction(p.context(), MANAGE).allowed()
        || authorization.authorizeAction(p.context(), CHANGE).allowed()
        || authorization.authorizeAction(p.context(), REVIEW).allowed()) {
      return v;
    }
  ...
  private static AuthorizationTarget target(ShiftChangeService.ShiftChange v) {
  ...
  private static void require(AuthorizationDecision d) {
    if (!d.allowed()) {
      throw new AccessDeniedException("P007 authorization denied: " + d.reason());
    }
  }
  ...
  private String hash(Object v) {
    try {
      ...
      throw new IllegalArgumentException("P007 request cannot be hashed", e);
    }
  }
}
