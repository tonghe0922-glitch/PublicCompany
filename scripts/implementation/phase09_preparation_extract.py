...
OUT_JSON = OUT_DIR / "P001_P005_SOURCE_SNAPSHOT.json"
OUT_MD = OUT_DIR / "P001_P005_SOURCE_SNAPSHOT.md"
...
def check_outputs() -> None:
    ...
    if not OUT_JSON.is_file() or not OUT_MD.is_file():
        raise RuntimeError("PHASE-09 source snapshot is missing")
    actual = json.loads(OUT_JSON.read_text(encoding="utf-8"))
    if actual != expected:
        raise RuntimeError("P001_P005_SOURCE_SNAPSHOT.json is stale")
    if OUT_MD.read_text(encoding="utf-8") != build_markdown(expected):
        raise RuntimeError("P001_P005_SOURCE_SNAPSHOT.md is stale")
...
def main() -> None:
    ...
    if args.check:
        check_outputs()
        print("PHASE-09 P001-P005 XLSX snapshot is deterministic and current")
        return
    write_outputs()
    print("PHASE-09 P001-P005: 15 XLSX workbooks parsed and preparation snapshot generated")
