...
/**
 * P010 notification contains workflow metadata only; exam, practical and qualification evidence
 * stay server-side.
 */
public final class Phase10P010NotificationHandler implements PlatformOutboxHandler {

  public static final String EVENT_TYPE = "P010_LEARNING_EVENT", AGGREGATE_TYPE = "P010_LEARNING";

  private static final String TEMPLATE = "P010_LEARNING_EVENT";
  ...
  public Phase10P010NotificationHandler(
      NotificationService notifications, JdbcTemplate jdbc, ObjectMapper mapper) {
    if (notifications == null || jdbc == null || mapper == null) {
      throw new IllegalArgumentException("P010 notification dependencies are required");
    }
  ...
  public String consumerName() {
    return "phase10-p010-notification";
  }
  ...
  public void handle(PlatformOutboxEvent event) {
    if (event == null
      ...
      throw new IllegalArgumentException("P010 outbox aggregate is invalid");
    }
    ...
    for (UUID recipient : recipients(p.path("recipientEmployeeIds"))) {
      notifications.create(
          new NotificationService.CreateCommand(
              event.tenantId(),
              null,
              "p010-notify:" + event.id() + ":" + recipient,
              TEMPLATE,
              "IN_APP",
              "EMPLOYEE",
              recipient,
...
""",
  ...
  private JsonNode parse(String raw) {
    try {
      ...
      if (n == null || !n.isObject()) {
        throw new IllegalArgumentException("P010 event payload must be an object");
      }
      return n;
    } catch (IllegalArgumentException e) {
      throw e;
    } catch (Exception e) {
      throw new IllegalArgumentException("P010 event payload is invalid JSON", e);
    }
  }
  ...
  private static String required(JsonNode n, String f) {
    ...
    if (v == null || !v.isTextual() || v.asText().isBlank()) {
      throw new IllegalArgumentException("P010 event field is required: " + f);
    }
    return v.asText();
  }
      ...
      n.forEach(
          v -> {
            try {
              ...
              throw new IllegalArgumentException("P010 recipient is invalid", e);
            }
          });
    }
    return ids;
  ...
  private static String label(String n) {
    return switch (n) {
      ...
      case "S10" -> "Archive";
      case "END" -> "Closed";
      default -> throw new IllegalArgumentException("P010 event node is not source-backed: " + n);
    };
  }
}
