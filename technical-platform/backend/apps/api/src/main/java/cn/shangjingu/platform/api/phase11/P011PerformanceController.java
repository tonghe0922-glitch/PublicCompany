...
public final class P011PerformanceController {
  ...
  private static final String MANAGE = "p011.performance.manage",
      READ = "p011.performance.read",
      MONITOR = "p011.performance.monitor";

  private final PerformanceCycleService performance;
  ...
  public P011PerformanceController(
      PerformanceCycleService performance,
  ...
  public Phase11AvailableActionProjectionService.RecordView<PerformanceCycleService.Cycle> create(
      @AuthenticationPrincipal SessionPrincipal p,
      @RequestHeader("Idempotency-Key") String key,
      @RequestBody PerformanceCycleService.CreateCommand c) {
    ...
    audit.recordOperationOnce(
        p.context(), "P011_CREATE_ATTEMPT", "performance.performance_cycle", null, key);
    var r = performance.create(context(p), key, hash(c), c);
    audit.recordOperationOnce(
        p.context(), "P011_CREATED", "performance.performance_cycle", r.id(), key);
    return view(p, r);
  }

  @GetMapping("/performance-cycles/{id}")
  public Phase11AvailableActionProjectionService.RecordView<PerformanceCycleService.Cycle> get(
      @AuthenticationPrincipal SessionPrincipal p, @PathVariable UUID id) {
    String permission = readPermission(p);
    var v =
        performance
            .find(context(p), id)
            .orElseThrow(() -> new IllegalArgumentException("P011 performance cycle not found"));
    require(authorization.authorizeData(p.context(), permission, target(v)));
    audit.recordOperation(p.context(), "P011_READ", "performance.performance_cycle", id);
    return view(p, v);
  }

  @GetMapping("/performance-cycles")
  public List<Phase11AvailableActionProjectionService.RecordView<PerformanceCycleService.Cycle>>
      list(@AuthenticationPrincipal SessionPrincipal p) {
    String permission = readPermission(p);
    var r =
        performance.list(context(p)).stream()
            .filter(v -> authorization.authorizeData(p.context(), permission, target(v)).allowed())
            .map(v -> view(p, v))
            .toList();
    audit.recordOperation(p.context(), "P011_LIST", "performance.performance_cycle", null);
    return r;
  }

  @PostMapping("/performance-cycles/{id}/actions/{actionCode}")
  public Phase11AvailableActionProjectionService.RecordView<PerformanceCycleService.Cycle> act(
      @AuthenticationPrincipal SessionPrincipal p,
      @PathVariable UUID id,
      @PathVariable String actionCode,
      @RequestHeader("Idempotency-Key") String key,
      @RequestBody PerformanceCycleService.ActionCommand c) {
    var current =
        performance
            .find(context(p), id)
            .orElseThrow(() -> new IllegalArgumentException("P011 performance cycle not found"));
    String action = safe(actionCode), permission = performance.permissionForAction(current, action);
    ...
    audit.recordOperationOnce(
        p.context(), "P011_ACTION_ATTEMPT_" + action, "performance.performance_cycle", id, key);
    var r =
        performance.act(
            context(p), id, action, key, hash(Map.of("actionCode", action, "body", c)), c);
    audit.recordOperationOnce(
        p.context(), "P011_ACTION_" + action, "performance.performance_cycle", id, key);
    return view(p, r);
  }

  private Phase11AvailableActionProjectionService.RecordView<PerformanceCycleService.Cycle> view(
      SessionPrincipal p, PerformanceCycleService.Cycle v) {
    boolean monitorOnly =
    ...
    return projections.project(
        ...
        v.versionNo(),
        () -> performance.availableActionCodes(context(p), v),
        code -> false,
        code -> performance.permissionForAction(v, code),
        target(v),
        monitorOnly);
  }
  ...
  private static AuthorizationTarget target(PerformanceCycleService.Cycle v) {
  ...
  private static void require(AuthorizationDecision d) {
    if (!d.allowed()) {
      throw new AccessDeniedException("P011 authorization denied: " + d.reason());
    }
  }
  ...
  private String hash(Object v) {
    try {
      ...
      throw new IllegalArgumentException("P011 request cannot be hashed", e);
    }
  }
}
