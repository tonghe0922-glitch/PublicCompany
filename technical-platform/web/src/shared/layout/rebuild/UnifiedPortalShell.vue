...
const searchResults = computed(() => {
  const home: ShellSearchItem = { sourceKey: 'portal-home', label: '首页', routePath: '/' }
  const items = [home, ...props.searchItems]
  const keyword = query.value.trim().toLocaleLowerCase('zh-CN')
  if (!keyword) return items.slice(0, 8)
  return items.filter((item) => item.label.toLocaleLowerCase('zh-CN').includes(keyword)).slice(0, 8)
})
  ...
  >
    ...
    <header class="rebuild-shell__topbar">
      ...
      <div class="rebuild-shell__topbar-actions">
        <button class="rebuild-shell__search-trigger" type="button" aria-label="搜索页面" @click="openSearch">
          <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="m20 20-3.5-3.5" /></svg>
        </button>
        <slot name="header" />
      </div>
    ...
    <main ref="mainRef" :id="contentId" class="rebuild-shell__main" tabindex="-1">
