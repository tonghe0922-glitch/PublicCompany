...
/** Real PHASE-10 / P010 Spring Boot + PostgreSQL16 + Redis browser fixture. */
public final class Phase10P010BrowserBackendFixture {

  private static final UUID TENANT = uuid("00000000-0000-0000-0000-000000002030"),
      CENTER_A = uuid("10000000-0000-0000-0000-000000003230"),
      CENTER_B = uuid("10000000-0000-0000-0000-000000003231"),
      POS_A = uuid("20000000-0000-0000-0000-000000003230"),
      POS_B = uuid("20000000-0000-0000-0000-000000003231");
  private static final UUID LEARNER = uuid("30000000-0000-0000-0000-000000003230"),
      MANAGER = uuid("30000000-0000-0000-0000-000000003231"),
      ASSESSOR = uuid("30000000-0000-0000-0000-000000003232"),
      CERTIFIER = uuid("30000000-0000-0000-0000-000000003233"),
      LINKER = uuid("30000000-0000-0000-0000-000000003234"),
      TECH = uuid("30000000-0000-0000-0000-000000003235"),
      OUT = uuid("30000000-0000-0000-0000-000000003236");
  private static final String API_PASSWORD = "p010_browser_api_" + shortId(),
      AUDIT_PASSWORD = "p010_browser_audit_" + shortId();

  private Phase10P010BrowserBackendFixture() {}

  public static void main(String[] args) throws Exception {
    String tenant = required("PHASE10_P010_TENANT"),
        managerLogin = required("PHASE10_P010_LOGIN"),
        password = required("PHASE10_P010_PASSWORD");
    ...
    System.out.println("PHASE10_P010_BROWSER_FIXTURE_READY");
    new CountDownLatch(1).await();
  }

  private static ConfigurableApplicationContext startApi(
      PostgreSQLContainer<?> p, GenericContainer<?> r) {
    return new SpringApplication(ApiApplication.class)
        .run(
            "--server.port=18090",
            "--spring.flyway.enabled=false",
            "--spring.datasource.url=" + url(p, "sjg_oms"),
            "--spring.datasource.username=sjg_api_runtime",
            "--spring.datasource.password=" + API_PASSWORD,
            "--spring.data.redis.host=" + r.getHost(),
            "--spring.data.redis.port=" + r.getMappedPort(6379),
            "--sjg.audit.datasource.url=" + url(p, "sjg_audit"),
  ...
  private static void prepare(
      PostgreSQLContainer<?> p, String tenant, String managerLogin, String password)
      throws Exception {
    Path root = root();
    Flyway.configure()
        .dataSource(p.getJdbcUrl(), p.getUsername(), p.getPassword())
        .locations("filesystem:" + root.resolve("technical-platform/database/flyway/cluster"))
        .cleanDisabled(true)
        .load()
        .migrate();
    try (Connection c =
            DriverManager.getConnection(p.getJdbcUrl(), p.getUsername(), p.getPassword());
    ...
    Flyway.configure()
        .dataSource(url(p, "sjg_oms"), p.getUsername(), p.getPassword())
        .locations(
        ...
        .migrate();
    Flyway.configure()
        .dataSource(url(p, "sjg_audit"), p.getUsername(), p.getPassword())
        .locations(
        ...
        .load()
        .migrate();
    seed(p, managerLogin, password);
  }

  private static void seed(PostgreSQLContainer<?> p, String managerLogin, String password)
      throws Exception {
    String hash = new BCryptPasswordEncoder(12).encode(password);
    try (Connection c =
            DriverManager.getConnection(url(p, "sjg_oms"), p.getUsername(), p.getPassword());
        Statement s = c.createStatement()) {
      s.execute(
      ...
      String[][] roles = {
        {
          "phase10.p010.learner",
          "SELF",
          "p010.learning.read,p010.learning.complete,p010.learning.exam"
        },
        {managerLogin, "CENTER", "p010.learning.read,p010.learning.manage"},
        {"phase10.p010.assessor", "CENTER", "p010.learning.read,p010.learning.certify"},
        {"phase10.p010.certifier", "CENTER", "p010.learning.read,p010.learning.certify"},
        {"phase10.p010.linker", "CENTER", "p010.learning.read,p010.learning.link"},
        {"phase10.p010.tech", "CENTER", "p010.learning.monitor"},
        {"phase10.p010.out", "CENTER", "p010.learning.read"}
      };
      ...
      s.execute(
          "insert into"
              ...
              + " select '"
              + TENANT
              + "','SAFETY-V1',id,'P010_SELF',true,'APPROVAL-P010-BROWSER','"
              + MANAGER
  ...
  private static void writeFacts(PostgreSQLContainer<?> p, GenericContainer<?> r) throws Exception {
    Path out =
        root()
            .resolve(
                "technical-platform/backend/apps/api/target/phase10-p010-fixture-runtime.json");
  ...
  private static String url(PostgreSQLContainer<?> p, String db) {
    return "jdbc:postgresql://" + p.getHost() + ":" + p.getMappedPort(5432) + "/" + db;
  }

  private static String required(String n) {
    String v = System.getenv(n);
    if (v == null || v.isBlank()) {
      throw new IllegalStateException("required environment missing: " + n);
    }
    return v;
  }

  private static UUID derived(int g, int i) {
    return uuid("%d0000000-0000-0000-0000-%012d".formatted(g, 3230 + i));
  }
