...
$runningContainerIds=@(docker ps -q)
if($LASTEXITCODE -ne 0){throw "DOCKER_PS_ALL_FAILED exit=$LASTEXITCODE"}
$runningTestcontainerIds=@(docker ps -q --filter 'label=org.testcontainers=true')
if($LASTEXITCODE -ne 0){throw "DOCKER_PS_TC_FAILED exit=$LASTEXITCODE"}
$runningRyukIds=@(docker ps -q --filter 'ancestor=testcontainers/ryuk:0.12.0')
if($LASTEXITCODE -ne 0){throw "DOCKER_PS_RYUK_FAILED exit=$LASTEXITCODE"}
$after=[ordered]@{captured_at_utc=(Get-Date).ToUniversalTime().ToString('o');classification='pre-existing active runtime / source-baseline non-writer';expected_process_ids=$expectedProcessIds;process_count=$processes.Count;processes=$processes;listener_count=$listeners.Count;listeners=$listeners;listener_ports=@($listeners | ForEach-Object {$_.port} | Sort-Object -Unique);running_container_count=$runningContainerIds.Count;running_container_ids=$runningContainerIds;running_testcontainers_count=$runningTestcontainerIds.Count;running_testcontainers_ids=$runningTestcontainerIds;running_ryuk_count=$runningRyukIds.Count;running_ryuk_ids=$runningRyukIds;status='PASS'}
...
$summary=[ordered]@{
    ...
    listener_ports_exact=(Compare-Object $expectedPorts $afterPorts).Count -eq 0;running_container_count=$after.running_container_count;running_testcontainers_count=$after.running_testcontainers_count;running_ryuk_count=$after.running_ryuk_count;candidate_mismatch_count=$candidateDiff.Count
}
