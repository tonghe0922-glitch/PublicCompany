...
/**
 * P009 progress notification; labor facts, scheme, receipt reference and payroll amount never enter
 * messages.
 */
public final class Phase10P009NotificationHandler implements PlatformOutboxHandler {

  public static final String EVENT_TYPE = "P009_OVERTIME_EVENT", AGGREGATE_TYPE = "P009_OVERTIME";

  private static final String TEMPLATE = "P009_OVERTIME_EVENT";
  ...
  public Phase10P009NotificationHandler(
      NotificationService notifications, JdbcTemplate jdbc, ObjectMapper mapper) {
    if (notifications == null || jdbc == null || mapper == null) {
      throw new IllegalArgumentException("P009 notification dependencies are required");
    }
  ...
  public String consumerName() {
    return "phase10-p009-notification";
  }
  ...
  public void handle(PlatformOutboxEvent event) {
    if (event == null
      ...
      throw new IllegalArgumentException("P009 outbox aggregate is invalid");
    }
    ...
    for (UUID recipient : recipients(p.path("recipientEmployeeIds"))) {
      notifications.create(
          new NotificationService.CreateCommand(
              event.tenantId(),
              null,
              "p009-notify:" + event.id() + ":" + recipient,
              TEMPLATE,
              "IN_APP",
              "EMPLOYEE",
              recipient,
...
""",
  ...
  private JsonNode parse(String raw) {
    try {
      ...
      if (n == null || !n.isObject()) {
        throw new IllegalArgumentException("P009 event payload must be an object");
      }
      return n;
    } catch (IllegalArgumentException e) {
      throw e;
    } catch (Exception e) {
      throw new IllegalArgumentException("P009 event payload is invalid JSON", e);
    }
  }
  ...
  private static String required(JsonNode n, String f) {
    ...
    if (v == null || !v.isTextual() || v.asText().isBlank()) {
      throw new IllegalArgumentException("P009 event field is required: " + f);
    }
    return v.asText();
  }
      ...
      n.forEach(
          v -> {
            try {
              ...
              throw new IllegalArgumentException("P009 recipient is invalid", e);
            }
          });
    }
    return ids;
  ...
  private static String label(String n) {
    return switch (n) {
      ...
      case "S09" -> "归档";
      case "END" -> "已关闭";
      default -> throw new IllegalArgumentException("P009 event node is not source-backed: " + n);
    };
  }
}
